...
//            _subsManager = subsManager ?? new InMemoryEventBusSubscriptionsManage();
...
//        public void Publish(IntegrationEvent model)
//        {
//            if (!_persistentConnection.IsConnected)
//            {
//                _persistentConnection.TryConnect();
...
//            var eventName = model.GetType().Name;

//            _logger.LogTrace("Creating RabbitMQ channel to publish event: {EventId} ({EventName})", model.Id, eventName);

//            using var channel = _persistentConnection.CreateModel();
//            _logger.LogTrace("Declaring RabbitMQ exchange to publish event: {EventId}", model.Id);
...
//        /// 订阅
//        /// </summary>
//        /// <typeparam name="TEvent">约束：事件模型</typeparam>
//        /// <typeparam name="THandle">约束：事件处理器<事件模型></typeparam>
...
//            _logger.LogInformation("Subscribing to event {EventName} with {EventHandler}", eventName, typeof(THandle).GetGenericTypeName());

//            ConsoleHelper.WriteSuccessLine($"Subscribing to event {eventName} with {typeof(THandle).GetGenericTypeName()}");
