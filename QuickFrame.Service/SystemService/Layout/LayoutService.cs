        ...
        private async Task<IEnumerable<MemberItem>> GetXElementAsync()
        {
            ...
            static IEnumerable<MemberItem> SelectMemberItem(XElement xml)
            {
                ...
                return members.Select(x => new MemberItem
                {
                    ...
                    Param = x?.Elements("param")?.Select(m => new NameValueItem { Name = m?.Attribute("name")?.Value ?? string.Empty, Value = m?.Value.Trim() ?? string.Empty }).ToArray() ?? Array.Empty<NameValueItem>(),
                    Returns = x?.Element("returns")?.Value.Trim() ?? string.Empty,
                    Summary = x?.Element("summary")?.Value.Trim() ?? string.Empty,
                    TypeParam = x?.Elements("typeparam")?.Select(m => new NameValueItem { Name = m?.Attribute("name")?.Value ?? string.Empty, Value = m?.Value.Trim() ?? string.Empty }).ToArray() ?? Array.Empty<NameValueItem>(),
                });
            }
